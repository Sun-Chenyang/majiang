...
[开发计划 §3](./library/03-开发计划.md)，算法规格见
[技术方案 §4](./library/02-技术方案设计.md)（§4.0 有现状与目标差距表），
算法结果如何接入 UI 见技术方案 §5.2 的接口契约。

## 文档地图（按需阅读，位于 library/）

| 文档 | 内容 |
|---|---|
| [library/01-产品需求说明.md](./library/01-产品需求说明.md) | PRD：功能需求、术语（向听数/进张/番型）、优先级 |
| [library/02-技术方案设计.md](./library/02-技术方案设计.md) | 架构、算法（胡牌判定/听牌枚举）、测试策略 |
| [library/03-开发计划.md](./library/03-开发计划.md) | 里程碑与任务拆分 |
| [library/卡五星规则.md](./library/卡五星规则.md) | 规则事实来源（番型总表、行牌规则），规则页数据整理自此 |
| **[library/04-UI设计规范.md](./library/04-UI设计规范.md)** | **设计系统：光照规则/色彩令牌/组件规范/滚动物理/资产规范/性能预算。任何 UI 改动前必读** |
